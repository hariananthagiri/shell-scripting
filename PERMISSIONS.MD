...
#### NOTE
    * **OWNERS & ROOT USERS can change the prtmissions of a file** 
...
```sh
id user_name    (to known both user_id & group_id)
id -u user_name (to known only user_id)
id -g user_name (to known only group_id)
```
